    ...
    <tbody>
        ...
        ?>
            <tr>
                ...
                <td>
                    <?php if (isset($_GET['cat']) && $_GET['cat'] === 'terhapus') { ?>
                        <button class="btn btn-warning btn-sm" onclick="confirm('User <?= $username; ?> Telah terhapus dan tidak dapat login \nApakah anda mau mengembalikan hak akses User ini ? ') && del('statuser','<?= $username; ?>','<?= $id; ?>','list_user&cat=terhapus','table-user','restore')">
                            <i class="fa-solid fa-undo"></i>
                        </button>
                    <?php } else { ?>
                        <button class="btn btn-warning btn-sm" onclick="route('form_edit_user&id=<?= $id; ?>','popupcontent','1','true');">
                            <i class="fa-solid fa-pen"></i>
                        </button>
                        <button class="btn btn-danger btn-sm" onclick="<?= $confirm ?>">
